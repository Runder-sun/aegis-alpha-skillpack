...
def main() -> int:
    home = env_home("HERMES_HOME", Path.home() / ".hermes")
    args = parse_args(home / "skills" / "aegis-alpha", "Install Aegis Alpha Hermes skill package.")
    copy_canonical_package(args.target.expanduser(), args.force)
    print_success("Hermes", args.target.expanduser())
    return 0
