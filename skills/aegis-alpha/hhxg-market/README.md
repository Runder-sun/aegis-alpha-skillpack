...
> Claude Code / OpenClaw 技能：零配置获取 A 股日报、日历、融资融券。
...
| 实时快讯（独立脚本 / API） | `news.py` | 财经快讯流（按时间倒序，不走 skill dispatch） |
...
> 运行时统一财经新闻入口已迁移到 `market-intel::daily-news-scan`。`news.py` 仍可作为独立脚本 / API 使用，但不再属于 `hhxg-market` 的 skill command surface。
...
python3 "$SKILL_DIR/news.py" 30                  # 独立快讯脚本（非 skill command）
...
| `GET /api/news?limit=20` | 最新财经快讯（最多 50 条，外部 API，不对应 hhxg-market skill command） |
...
3. 保存，在 GPT System Prompt 中加入：
   ```
   你是 A 股量化数据助手。用户询问 A 股行情、连板、龙虎榜、融资融券时，
   调用 getSnapshot/getMargin/getCalendar 获取数据；如需财经新闻，调用外部
   getNews API（不对应 hhxg-market skill command）。数据每个交易日盘后 20:00 更新。
   ```
...
│   ├── calendar.py           # A 股日历
│   ├── margin.py             # 融资融券
│   └── news.py               # 实时快讯（独立脚本 / API，不暴露为 skill command）
└── references/
