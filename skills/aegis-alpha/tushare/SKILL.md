...
以下命令由 `scripts/dispatch.py` 提供，适合在 pipeline/skill 内部调用：

### news
新闻快讯（短讯）

Payload:
- `src` (string, optional) — 新闻来源，如 `sina`/`wallstreetcn`/`eastmoney`
- `start_date` (string, optional) — 起始日期 `YYYYMMDD HH:MM:SS`
- `end_date` (string, optional) — 结束日期 `YYYYMMDD HH:MM:SS`
- `limit` (int, optional) — 返回条数，default `50`

### major-news
新闻通讯（长文财经新闻）

Payload:
- `src` (string, optional) — 来源筛选
...
Payload:
- `ts_code` (string, optional) — 股票代码 `000001.SZ`，空则返回全市场
- `report_type` (string, optional) — 报告类型 `1`(研报)/`2`(策略)/`3`(宏观)
- `start_date` (string, optional) — `YYYYMMDD`
- `end_date` (string, optional)
...
Payload:
- `date` (string, optional) — 指定日期 `YYYYMMDD`，默认今日
- `start_date` (string, optional)
- `end_date` (string, optional)

### macro-cn-m2
国内货币供应量（M2/M1/M0）月度时序

Payload:
- `start_m` (string, optional) — 开始月份 `YYYYMM`
- `end_m` (string, optional) — 结束月份 `YYYYMM`
- `limit` (int, optional) — default `24`

### macro-cn-sf
社融增量（月度）

Payload:
- `start_m` (string, optional) — `YYYYMM`
- `end_m` (string, optional)
- `limit` (int, optional) — default `24`
...
Payload:
- `start_q` (string, optional) — 季度 `YYYY1Q`/`YYYY2Q`/`YYYY3Q`/`YYYY4Q`
- `end_q` (string, optional)
- `limit` (int, optional) — default `20`
...
Payload:
- `date` (string, optional) — `YYYYMMDD`，默认最新
- `start_date` (string, optional)
- `end_date` (string, optional)
- `limit` (int, optional) — default `30`
...
### daily-basic
A 股每日基础指标截面数据：PE/PB/换手率/量比/总市值/流通市值等。适合筛选低估值、高换手标的。

Payload:
- `ts_code` (string, optional) — 股票代码 `000001.SZ`；空则返回全市场快照
- `trade_date` (string, optional) — 交易日 `YYYYMMDD`，默认最新
- `fields` (string, optional) — 返回字段，default `ts_code,trade_date,pe,pe_ttm,pb,ps,ps_ttm,dv_ratio,dv_ttm,total_mv,circ_mv,turnover_rate,volume_ratio`

Underlying API: `pro.daily_basic`

### stock-list
获取上市/退市/暂停交易的股票基础列表，包含行业/地区/上市日期等字段。

Payload:
- `list_status` (string, optional) — `L`(上市)|`D`(退市)|`P`(暂停)，default `L`
- `fields` (string, optional) — default `ts_code,symbol,name,area,industry,market,list_date,is_hs`

Underlying API: `pro.stock_basic`

### stk-limit
每日涨跌停价格及当日炸板/连板数据。

Payload:
- `ts_code` (string, optional) — 股票代码；空则返回全市场
- `trade_date` (string, optional) — `YYYYMMDD`，默认最新

Underlying API: `pro.stk_limit`

### top-list
龙虎榜每日数据（异动个股上榜原因及净买入）。

Payload:
- `trade_date` (string, optional) — `YYYYMMDD`，默认最新
- `ts_code` (string, optional) — 过滤指定股票

Underlying API: `pro.top_list`
...
- `trade_date` (string, optional) — `YYYYMMDD`，默认最新
- `ts_code` (string, optional) — 过滤指定股票
- `exchange_id` (string, optional) — `SSE`|`SZSE`

Underlying API: `pro.margin`
...
- `ann_date` (string, optional) — 公告日期 `YYYYMMDD`
- `float_date` (string, optional) — 解禁日期 `YYYYMMDD`
- `limit` (int, optional) — default `30`

Underlying API: `pro.share_float`

### income
利润表数据（营业收入/净利润/毛利等）。

Payload:
- `ts_code` (string, required) — 股票代码 `000001.SZ`
- `period` (string, optional) — 报告期 `YYYYMMDD`（如 `20231231`）
- `report_type` (string, optional) — `1`(合并)|`2`(单体)，default `1`
- `limit` (int, optional) — 最近 N 期，default `8`

Underlying API: `pro.income`

### balancesheet
资产负债表数据（总资产/总负债/股东权益等）。

Payload:
- `ts_code` (string, required) — 股票代码 `000001.SZ`
- `period` (string, optional) — 报告期 `YYYYMMDD`
- `limit` (int, optional) — default `8`

Underlying API: `pro.balancesheet`
...
Underlying API: `pro.cashflow`

### fina-indicator
综合财务指标数据：ROE/ROA/毛利率/速动比率/资产负债率等。
...
Underlying API: `pro.fina_indicator`

### sw-daily
申万行业指数日行情数据。

Payload:
- `ts_code` (string, optional) — 申万行业代码；空则返回全部行业
- `trade_date` (string, optional) — `YYYYMMDD`，默认最新
- `start_date` (string, optional)
- `end_date` (string, optional)

Underlying API: `pro.sw_daily`
...
|  337 | fut_weekly_monthly | [期货周/月线行情(每日更新)](references/期货数据/期货周月线行情(每日更新).md)                | 期货数据               | 期货周/月线行情(每日更新)                                                                                                                                                                      |
...
|  315 | realtime_quote     | [实时Tick（爬虫）](references/股票数据/行情数据/实时Tick(爬虫).md)                  | 股票数据,行情数据          | 本接口是tushare org版实时接口的顺延，数据来自网络，且不进入tushare服务器，属于爬虫接口，请将tushare升级到1.3.3版本以上。                                                                                                         |
|  316 | realtime_tick      | [实时成交（爬虫）](references/股票数据/行情数据/实时成交(爬虫).md)                      | 股票数据,行情数据          | 本接口是tushare org版实时接口的顺延，数据来自网络，且不进入tushare服务器，属于爬虫接口，数据包括该股票当日开盘以来的所有分笔成交数据。                                                                                                        |
|  317 | realtime_list      | [实时排名（爬虫）](references/股票数据/行情数据/实时排名(爬虫).md)                      | 股票数据,行情数据          | 本接口是tushare org版实时接口的顺延，数据来自网络，且不进入tushare服务器，属于爬虫接口，数据包括该股票当日开盘以来的所有分笔成交数据。                                                                                                        |
|  336 | stk_weekly_monthly | [周/月线行情(每日更新)](references/股票数据/行情数据/周月线行情(每日更新).md)               | 股票数据,行情数据          | 股票周/月线行情(每日更新)                                                                                                                                                                      |
|  365 | stk_week_month_adj | [周/月线复权行情(每日更新)](references/股票数据/行情数据/周月线复权行情(每日更新).md)           | 股票数据,行情数据          | 股票周/月线行情(复权--每日更新)                                                                                                                                                                  |
...
## Runtime Notes
### Env
- Requires `TUSHARE_TOKEN` in workspace `.env`.

### Failure & Fallback
- If token missing or quota insufficient, commands return empty data with error notes.
